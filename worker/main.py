    ...
    for item in data:
        if item['id'] == req.id:
            ...
            if 'history' in item and len(item['history']) > 1:
                item['history'].pop()
                last = item['history'][-1]
                item['current_price_per_round'] = last.get('price_per_round', item.get('current_price_per_round'))
            
            # Force immediate update
            item['last_update'] = "2020-01-01T00:00:00" 
            break
...
def auto_repair(req: UrlRequest):
    ...
    for item in data:
        ...
        if len(history) >= 3:
            prices = sorted([h['price_per_round'] for h in history])
            ...
            for i, h in enumerate(history):
                ...
                if median > 0 and abs(h['price_per_round'] - median) / median > 0.20:
                    fixed = True
                    if i == len(history) - 1:
                        removed_latest = True
                else:
...
def cleanup_duplicates(req: UrlRequest):
    ...
    for item in data:
        ...
        new_id = generate_id(str(caliber), str(brand), str(shop), str(amount))
        item['id'] = new_id
        ...
        if new_id in new_data:
            ...
            # Take latest price
            if item.get('last_update', '') > new_data[new_id].get('last_update', ''):
                new_data[new_id]['current_price_per_round'] = item.get('current_price_per_round', 0)
                new_data[new_id]['last_update'] = item.get('last_update', '')
            merged_count += 1
        else:
