...
def ask_gemini_validation(html_text, url):
    ...
    prompt = f"""
    Du bist ein Web-Klassifikator.
    Hier ist der rohe Text der Startseite einer Webseite ({url}).
    
    AUFGABE: Ist das ein echter Schweizer Online-Shop (E-Commerce), der Feuerwaffen oder echte Munition (wie 9mm, .223 Rem, etc.) verkauft?
    - Es MUSS ein Shop sein (mit Warenkorb / Preisen / Angeboten), keine reine Informationsseite.
    - Es MUSS in der Schweiz angesiedelt sein oder in die Schweiz liefern (oft erkennbar an .ch, CHF-Preisen oder Adressen).
    - Es MUSS Feuerwaffen oder scharfe Munition verkaufen (kein reiner Airsoft-, Paintball- oder Messer-Shop).
    
    Antworte NUR mit einem JSON-Objekt mit exakt diesem Schlüssel:
    {{"is_ammo_shop": true}} oder {{"is_ammo_shop": false}}
    ...
    try:
        ...
        print(f"Fehler bei der KI-Validierung für {url}: {e}")
        return False
...
def run_scout():
    ...
    print("Suche nach neuen URLs über DuckDuckGo...")
    try:
        with DDGS() as ddgs:
        ...
        print(f"Fehler bei DuckDuckGo Suche: {e}")
    ...
    print(f"{len(new_base_urls)} potenziell neue Basis-URLs gefunden.")
    ...
    for base_url in new_base_urls:
        print(f"\nÜberprüfe potenziellen neuen Shop: {base_url} ...")
        try:
            ...
            if is_shop:
                print(f" -> [ERFOLG] KI bestätigt: {base_url} ist ein Munitions-Shop!")
                ...
                print(f" -> [ABGELEHNT] KI sagt: Kein passender Shop.")
                
            time.sleep(2)
        except Exception as e:
            print(f" -> [FEHLER] Konnte {base_url} nicht prüfen: {e}")

    print(f"[{datetime.now().isoformat()}] SCOUT beendet. {new_shops_added} neue Shops hinzugefügt.")
